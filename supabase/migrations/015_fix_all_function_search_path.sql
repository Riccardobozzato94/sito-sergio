-- ═══════════════════════════════════════════════════════════
-- 015 — Fix ALL function search_path (schema-qualify tables)
-- Panificio Da Sergio — CRM Database
--
-- Tutte le funzioni con set search_path = '' non trovano
-- tabelle senza prefisso public.*. Questo fix applica
-- public. a tutti i riferimenti nelle funzioni di sistema.
-- ═══════════════════════════════════════════════════════════

-- ── Loyalty: add points (fix: public.customers) ──
...
-- ── Loyalty: redeem points (fix: public.customers) ──
...
-- ── Auto-calculate order total (fix: public.order_items, public.orders) ──
...
-- ── Daily analytics rollup (fix: all table refs) ──
...
-- ── Customer stats update (fix: public.customers, public.orders) ──
...
-- ── Full-text search products (fix: public.products) ──
...
-- ── Full-text search customers (fix: public.customers) ──
